		...
		<view v-if="show">
			<form @submit="formSubmit" @reset="" enctype="multipart/form-data">
				...
				<view class="cu-form-group margin-top">
					...
					<input bindinput="getTitleValue" name="title" v-model="shop_all.name" placeholder="请输入商品名称"></input>
				</view>
				<!-- end -->
				...
				<view class="cu-form-group margin-top">
					...
					<textarea name="content" maxlength="1000" v-model="shop_all.text" placeholder="请输入商品描述"></textarea>
				</view>
				<!-- end -->
				...
				<view class="cu-form-group margin-top">
					...
					<input type="digit" v-model="shop_all.price" placeholder="请输入价钱" maxlength='7'
						name="newPrice"></input>
				</view>
				<!-- end -->
				...
				<view class="cu-form-group margin-top">
					...
					<input bindinput="getTitleValue" name="title" v-model="shop_all.tel" placeholder="请输入手机号码"
						type="number"></input>
				</view>
				<!-- end -->
				<!-- 选择分类  -->
				...
				<view class="cu-form-group margin-top">
					...
					<input bindinput="getTitleValue" name="title" v-model="shop_all.dingdan" placeholder="请输入订单号"
						type="number"></input>
				</view>
				<!-- end -->
	...
	export default {
		...
		methods: {
			...
			formSubmit() {
				...
				if (!this.shop_all.tel) { //判断如果手机号为空，提示用户输入手机号
					uni.showToast({
						title: '请输入手机号',
						icon: 'none'
					})
					return
				} else if (!reg.test(this.shop_all.tel)) { //判断手机号格式是否正确
					uni.showToast({
						title: '请输入正确的手机号',
						icon: 'none'
					})
					return
				} else if (!this.shop_all.name) {
					uni.showToast({
						title: '请输入商品简介',
						icon: 'none'
					})
					return
				} else if (!this.shop_all.text) {
					uni.showToast({
						title: '请输入商品简介',
						icon: 'none'
					})
					return
				} else if (!this.shop_all.price) {
					uni.showToast({
						title: '请输入商品简介',
						icon: 'none'
					})
				...
				let submit = {
					...
					commodity_name: this.shop_all.name,
					commodity_image: this.imgList.join(','),
					commodity_price: this.shop_all.price,
					commodity_content: this.shop_all.text,
					tel: this.shop_all.tel,
					commodity_category_id: this.shop_id,
					receipt: this.shop_all.dingdan,
					province: '1',
					city: this.region,
					area: '1'
				}
				...
				this.$request('secondhand/add', submit).then(res => {
					console.log(res)
					if (res.code == 1) {
						this.show = false
						this.shop_all = {}
